...
from torch.nn import Linear, GRU, Embedding
...
class ImdbNet(Module):
    ...
    def forward(self, x):
        ...
        x = self.embedding(x)
        x = x.permute(1, 0, 2)  # x经过permute将变成 (seq_len, batch_size, input_size), 便于适应LSTM输入，GRU和RNN也要
        x, _ = self.gru(x,prev_h) # GRU只有一个隐藏状态prev_h
