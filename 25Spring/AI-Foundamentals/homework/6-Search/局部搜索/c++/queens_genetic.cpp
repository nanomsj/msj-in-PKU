	...
	void show() const override {
		for (int i = 0; i < _population.size(); ++ i){
			if (_adaptability[i] == 0){
				...
				std::cout << std::endl;
				std::cout << "Found solution at generation with adaptability 0." << std::endl;
				
				// 输出耗时（调用主程序计时变量）
				extern time_t t0;
				std::cout << "Total time: " << time(nullptr) - t0 << "s" << std::endl;
